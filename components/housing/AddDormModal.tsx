...
const EMPTY: DormForm = {
  name: '', location: '', type: 'dormitory', total_capacity: '',
  number_of_semesters_allowed: '', curfew_time: '',
  allowed_programs: '', term_type: 'semestral',
  separate_by_gender: true, manager_id: '',
}

export default function AddDormModal({ isOpen, onClose, onSuccess, existingDorm }: Props) {
  ...
  useEffect(() => {
    if (existingDorm) {
      setForm({
        ...
        number_of_semesters_allowed: String(existingDorm.dormitory?.number_of_semestersAllowed ?? ''),
  ...
  async function handleSubmit() {
    ...
    try {
      if (isEditing && existingDorm) {
        // ── UPDATE ──────────────────────────────────────────────────────────
        const res = await fetch(`/api/admin/housing/dorms?id=${existingDorm.accommodation_id}`, {
          method:  'PATCH',
        ...
        const res = await fetch('/api/admin/housing/dorms', {
          ...
          body:    JSON.stringify({
            ...
            manager_id:                  form.manager_id,
            total_capacity:              Number(form.total_capacity),
            number_of_semesters_allowed: Number(form.number_of_semesters_allowed),
            curfew_time:                 form.curfew_time || null,
            allowed_programs:            form.allowed_programs || null,
            term_type:                   form.term_type,
            separate_by_gender:          form.separate_by_gender,
    ...
    >
      ...
      <div className="space-y-4">
          ...
          <>
            <FLabel label="Dorm Name" required>
              <input name="name" value={form.name} onChange={handleChange} required
                className={inputCls} placeholder="e.g. Sampaguita Dormitory" />
            </FLabel>
            <FLabel label="Location" required>
              <input name="location" value={form.location} onChange={handleChange} required
                className={inputCls} placeholder="e.g. Main Campus, Building C" />
            </FLabel>
            <FLabel label="Total Capacity" required>
              <input name="total_capacity" type="number" value={form.total_capacity}
                onChange={handleChange} required className={inputCls} placeholder="e.g. 40" />
            </FLabel>
          </>
        )}
        ...
        {step === 2 && (
          <>
            <FLabel label="Number of Semesters Allowed" required>
              <input name="number_of_semesters_allowed" type="number"
                value={form.number_of_semesters_allowed} onChange={handleChange}
                required className={inputCls} placeholder="e.g. 2" />
            </FLabel>
            <FLabel label="Term Type" required>
              <select name="term_type" value={form.term_type} onChange={handleChange} className={inputCls}>
                <option value="semestral">Semestral</option>
                <option value="annual">Annual</option>
              </select>
            </FLabel>
            <FLabel label="Curfew Time">
              <input name="curfew_time" type="time" value={form.curfew_time}
                onChange={handleChange} className={inputCls} />
            </FLabel>
            <FLabel label="Allowed Programs">
              <textarea name="allowed_programs" value={form.allowed_programs}
                onChange={handleChange} rows={2}
                className={inputCls} placeholder="e.g. All programs, or BS Computer Science only" />
            </FLabel>
            <label className="flex items-center gap-3 text-sm">
              <input type="checkbox" name="separate_by_gender"
                checked={form.separate_by_gender} onChange={handleChange}
        ...
        {step === 4 && !isEditing && (
          <div className="rounded-lg bg-blue-50 p-4 text-sm text-blue-700">
            ...
            <p>You can add units/rooms from the property detail page after creation.</p>
          </div>
        )}
      ...
      <div className="flex justify-between mt-6">
        ...
        {step < totalSteps ? (
          ...
          >
            {loading ? 'Saving...' : isEditing ? 'Save Changes' : 'Create Dormitory'}
          </button>
        )}
      </div>
    </Modal>
...
function FLabel({ label, required = false, children }: {
