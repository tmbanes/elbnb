...
const EMPTY: RentalForm = {
  ...
  allow_shortterm_stay: false, allow_longterm_stay: true,
  minimum_stay_days: '', maximum_stay_days: '',
  security_deposit_required: false, manager_id: '',
}

export default function AddRentalSpaceModal({ isOpen, onClose, onSuccess, existingRental }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      if (isEditing && existingRental) {
        // ── UPDATE ──────────────────────────────────────────────────────────
        const res = await fetch(`/api/admin/housing/rental-spaces?id=${existingRental.accommodation_id}`, {
          method:  'PATCH',
          ...
          body:    JSON.stringify({
            ...
            rentingFields: {
              ...
              minimum_stay_days:         form.minimum_stay_days ? Number(form.minimum_stay_days) : null,
              maximum_stay_days:         form.maximum_stay_days ? Number(form.maximum_stay_days) : null,
      ...
      <div className="flex items-center gap-2 mb-6">
        {Array.from({ length: totalSteps }).map((_, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            {i < totalSteps - 1 && (
              <div className={`h-0.5 w-8 ${step > i+1 ? 'bg-green-500' : 'bg-gray-200'}`} />
            )}
          </div>
        ))}
        <span className="ml-2 text-xs text-gray-500">Step {step} of {totalSteps}</span>
      </div>
          ...
          <>
            <div>
              ...
              <input name="name" value={form.name} onChange={handleChange}
                className={inputCls} placeholder="e.g. Mabini Boarding House" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <input name="location" value={form.location} onChange={handleChange}
                className={inputCls} placeholder="e.g. Near East Gate" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Total Capacity <span className="text-red-500">*</span>
              </label>
              <input name="total_capacity" type="number" value={form.total_capacity}
                onChange={handleChange} className={inputCls} placeholder="e.g. 10" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
        ...
        {step === 2 && (
          <>
            <label className="flex items-center gap-3 text-sm">
              <input type="checkbox" name="allow_shortterm_stay"
                checked={form.allow_shortterm_stay} onChange={handleChange} className="w-4 h-4 rounded" />
              <span>Allow Short-Term Stay</span>
            </label>
            <label className="flex items-center gap-3 text-sm">
              <input type="checkbox" name="allow_longterm_stay"
                checked={form.allow_longterm_stay} onChange={handleChange} className="w-4 h-4 rounded" />
            ...
            {form.allow_shortterm_stay && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="minimum_stay_days" type="number" value={form.minimum_stay_days}
                    onChange={handleChange} className={inputCls} placeholder="e.g. 7" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Max Stay Days</label>
                  <input name="maximum_stay_days" type="number" value={form.maximum_stay_days}
                    onChange={handleChange} className={inputCls} placeholder="e.g. 90" />
                </div>
              </div>
            )}

            <label className="flex items-center gap-3 text-sm">
              <input type="checkbox" name="security_deposit_required"
                checked={form.security_deposit_required} onChange={handleChange} className="w-4 h-4 rounded" />
        ...
        {step === 3 && (
          <>
            <div>
              ...
              <select name="manager_id" value={form.manager_id}
                onChange={handleChange} className={inputCls}>
            ...
            {form.manager_id && (
              <p className="text-xs text-green-600 bg-green-50 rounded p-2">✓ Manager selected</p>
            )}
          </>
        )}
