...
export default function AdminResidentsPage() {
  ...
  // Reset confirm when switching residents
...
function ResidentDetailPanel({
  ...
  const isAwaiting = s === "waiting_payment" || s === "pending";
          ...
          <div className="relative pl-7 space-y-6 before:absolute before:left-[9px] before:top-2 before:bottom-2 before:w-[2px] before:bg-[#e8e2d6]">
            ...
            {(isActive || isCheckedOut) && (
              <TimelineEvent
                color="bg-[#78A24C]"
                title="Moved In"
                subtitle={fmtDate(resident.move_in_date)}
              />
            )}
            {isCheckedOut && (
              <TimelineEvent
        ...
        {confirmAction ? (
          <div className={cn("bg-[#FDFFF4] border rounded-2xl p-5 shadow-lg space-y-4", confirmBorderColor)}>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={onConfirm}
                disabled={actionLoading || (confirmAction === "override" && !targetUnit)}
              ...
              >
                {actionLoading
                  ...
                  : confirmAction === "override" ? "Confirm Transfer" : "Confirm Action"}
              </button>
            </div>
          </div>
        ) : (
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 border border-[#e8e2d6] text-[#44291B] font-bold py-2.5 rounded-xl text-sm hover:bg-[#e8e2d6]/40 transition-colors bg-[#FDFFF4]"
                  >
                    <CalendarArrowUp className="w-4 h-4" /> Move-out
                  </button>
                  <button
                    onClick={() => onOpenConfirm("terminate")}
                    className="flex items-center justify-center gap-2 border border-[#e8e2d6] text-[#DF3538] font-bold py-2.5 rounded-xl text-sm hover:bg-red-50 transition-colors bg-[#FDFFF4]"
                  >
                    Terminate <ShieldAlert className="w-4 h-4" />
                  </button>
                </div>
                {showOverride && (
                  <button
                    onClick={() => onOpenConfirm("override")}
                    className="w-full border border-dashed border-amber-300 text-amber-700 font-bold py-3 rounded-xl text-xs hover:bg-amber-50 transition-colors flex items-center justify-center gap-2"
                  >
                    <RefreshCw className="w-3.5 h-3.5" /> Admin Override: Transfer to Different Unit
                  </button>
                )}
              </div>
            )}
