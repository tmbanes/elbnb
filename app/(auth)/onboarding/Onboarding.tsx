...
export default function Auth() {
  ...
  return (
    <div 
      className="min-h-screen bg-[#8dbd59] flex items-center justify-center p-4" 
          ...
          <div className="w-[45%] h-[100%] grid grid-cols-2 gap-1 relative">
            <div className={`absolute inset-y-0 left-0 w-1/2 bg-[#fcf4d9] border-[#44291B] transition-transform duration-700 z-10 grid grid-cols-2 grid-rows-2 p-1 gap-1 ${upperOpen ? '-translate-x-full' : 'translate-x-0'}`}>
              ...
              <div className="absolute right-0.5 top-1/2 -translate-y-1/2 flex items-center">
                <button onClick={(e) => { e.stopPropagation(); setUpperOpen(!upperOpen); }} className="w-10 h-10 rounded-full bg-[#3e2319] border-4 border-[#fcf4d9] text-[#fcf4d9] text-xs flex items-center justify-center hover:bg-[#2d1a12] transition-colors">
                  {upperOpen ? '→' : '←'}
                </button>
              </div>
            </div>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center z-0 p-4 gap-2">
              {upperOpen && (
                <>
                  ...
                  <button onClick={() => window.location.href = '/signup'} className="px-20 py-3 bg-[#fbbc05] text-[#1a1a1a] rounded-full font-semibold hover:bg-[#fcf4d9] transition-all">
                    Sign Up
                  </button>
                </>
              )}
        ...
        >
          <div className="w-[45%] h-[100%] grid grid-cols-2 gap-1 relative">
            ...
            <div className={`absolute inset-y-0 left-0 w-1/2 bg-[#fcf4d9] transition-transform duration-700 z-10 grid grid-cols-2 grid-rows-2 p-1 gap-1 ${lowerOpen ? '-translate-x-full' : 'translate-x-0'}`}>
              ...
              <div className="absolute right-0.5 top-1/2 -translate-y-1/2 flex items-center">
                <button onClick={(e) => { e.stopPropagation(); setLowerOpen(!lowerOpen); }} className="w-10 h-10 rounded-full bg-[#3e2319] border-4 border-[#fcf4d9] text-[#fcf4d9] text-xs flex items-center justify-center hover:bg-[#2d1a12] transition-colors">
                  {lowerOpen ? '→' : '←'}
                </button>
              </div>
            </div>
                ...
                <>
                  ...
                  <div className="flex flex-col gap-3 w-full items-center">
                    ...
                    <button
                      ...
                      className="w-full max-w-[240px] h-10 px-4 bg-[#fbbc05] text-[#2d1a12] rounded-full hover:bg-[#fcf4d9] transition-all text-sm font-medium"
                    >
                      Log In with Google
                    </button>
                  </div>
