...
export const POST = withRole(['student'], async (req: NextRequest) => {
  try {
    ...
    const storagePath = `${user.id}/${billing.billing_id}/${receiptId}.${fileExtension}`;
    ...
    const result = await submitPayment(user.id, billing.billing_id, storagePath, receiptId, "cash");
