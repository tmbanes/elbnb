...
function ManagersContent() {
  ...
  useEffect(() => {
    if (!selectedId) { setSelectedManager(null); return }
  ...
  async function handleDelete(employeeId: string) {
    ...
    const res  = await fetch(`/api/admin/housing/managers?id=${employeeId}`, { method: 'DELETE' })
    const data = await res.json()
    ...
    return (
      <div className="p-6 space-y-6">
          ...
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-2xl font-bold">
                {selectedManager.users.first_name} {selectedManager.users.last_name}
              </h1>
        ...
        <AddManagerModal
          isOpen={modalOpen}
          onClose={closeModal}
          onSuccess={() => { fetchManagers(); closeModal() }}
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      {managers.length === 0 ? (
        <p className="text-gray-500">No managers yet. Add one to get started.</p>
      ) : (
...
function AssignedProperties({ managerId }: { managerId: string }) {
  ...
  return (
    <div className="rounded border p-4">
      <h2 className="font-semibold mb-3">Assigned Properties ({properties.length})</h2>
      {properties.length === 0 ? (
        ...
        <ul className="space-y-2">
          {properties.map(p => (
            ...
            >
              <span>{p.name} — <span className="text-gray-500">{p.location}</span></span>
              <button
                onClick={() => router.push(`/dashboard/admin/housing/properties?id=${p.accommodation_id}`)}
                className="text-blue-600 hover:underline text-xs"
              >
                View
              </button>
