...
function PropertiesContent() {
  ...
  useEffect(() => {
    if (!selectedId) { setSelectedProperty(null); return }

    async function fetchDetail() {
      setDetailLoading(true)
      try {
        const found    = properties.find(p => p.accommodation_id === selectedId)
        const endpoint = found?.accommodation_type === 'renting_space'
  ...
  async function handleDeleteProperty(id: string, type: string) {
    ...
    const endpoint = type === 'renting_space'
  ...
  async function handleDeleteUnit(unitId: string) {
    ...
    const res  = await fetch(`/api/admin/housing/units?id=${unitId}`, { method: 'DELETE' })
    const data = await res.json()
  ...
  if (selectedId) {
    ...
    return (
      <div className="p-6 space-y-6">
        <button onClick={handleBackToList} className="text-blue-600 hover:underline">
          ← Back to Properties
        </button>
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <span className={`text-xs px-2 py-1 rounded mt-1 inline-block font-medium ${
              isDorm
        ...
        {isDorm && selectedProperty.dormitory && (
          <div className="rounded border p-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <Detail label="Semesters Allowed"  value={String(selectedProperty.dormitory.number_of_semestersAllowed)} />
              <Detail label="Curfew Time"         value={selectedProperty.dormitory.curfew_time || '—'} />
              <Detail label="Term Type"           value={selectedProperty.dormitory.term_type} />
              <Detail label="Separate by Gender"  value={selectedProperty.dormitory.separate_by_gender ? 'Yes' : 'No'} />
              <Detail label="Allowed Programs"    value={selectedProperty.dormitory.allowed_programs || '—'} />
            </div>
          </div>
        )}
        ...
        {!isDorm && selectedProperty.renting_space && (
          <div className="rounded border p-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <Detail label="Property Type"    value={selectedProperty.renting_space.property_type} />
              <Detail label="Short-Term Stay"  value={selectedProperty.renting_space.allow_shortterm_stay ? 'Allowed' : 'Not Allowed'} />
              <Detail label="Long-Term Stay"   value={selectedProperty.renting_space.allow_longterm_stay  ? 'Allowed' : 'Not Allowed'} />
              <Detail label="Min Stay Days"    value={String(selectedProperty.renting_space.minimum_stay_days ?? '—')} />
              <Detail label="Max Stay Days"    value={String(selectedProperty.renting_space.maximum_stay_days ?? '—')} />
              <Detail label="Security Deposit" value={selectedProperty.renting_space.security_deposit_required ? 'Required' : 'Not Required'} />
            </div>
          </div>
        )}
              ...
              <tbody>
                {selectedProperty.units.map(unit => (
                  <tr key={unit.unit_id} className="border-b">
                    ...
                    <td>{unit.current_occupancy} / {unit.max_occupancy}</td>
        ...
        <AddDormModal
          ...
          onSuccess={() => { fetchProperties(); closeDormModal() }}
          existingDorm={
            editingProperty?.accommodation_type === 'dormitory' ? editingProperty : null
          }
        />
        <AddRentalSpaceModal
          isOpen={rentalModalOpen}
          onClose={closeRentalModal}
          onSuccess={() => { fetchProperties(); closeRentalModal() }}
          existingRental={
            editingProperty?.accommodation_type === 'renting_space' ? editingProperty : null
          }
        />
      </div>
    )
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 items-center">
          ...
          {(['', 'dormitory', 'renting_space'] as const).map(type => (
            <button
              key={type}
              onClick={() => router.push(type ? `?type=${type}` : '/dashboard/admin/housing/properties')}
      ...
      {filtered.length === 0 ? (
        ...
        <table className="w-full text-sm border rounded">
          ...
          <tbody>
            {filtered.map(p => (
              <tr key={p.accommodation_id} className="border-b hover:bg-gray-50">
