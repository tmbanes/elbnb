...
export async function createBillingWithItems(
  ...
  const { error: insertItemsError } = await supabase.from("billing_item").insert(formattedItems);

  if (insertItemsError) {
    await supabase.from("billing").delete().eq("billing_id", billing.billing_id);
    return { data: null, error: insertItemsError };
  }
...
export async function updateAdminInvoiceWithItems(
  role: UserRole,
  billing_id: string,
  updates: {
    internal_notes?: string;
  ...
  const sanitizedItems = (items ?? []).filter(
    (item) => Number.isFinite(Number(item?.amount)) && Number(item.amount) > 0 && item.type,
  );
  ...
  const totalAmount = sanitizedItems.reduce((sum, item) => sum + Number(item.amount), 0);
  ...
  const { error: insertItemsError } = await supabase.from("billing_item").insert(
