...
export async function getAllManagers() {
  return supabaseAdmin
    ...
    .select(`employee_id, office_location, users (user_id, first_name, last_name, email)`)
}
...
export async function updateDormitory(
  ...
  accommodationFields: Partial<{ name: string; location: string; manager_id: string; total_capacity: number }>,
  dormitoryFields: Partial<{ number_of_semestersAllowed: number; curfew_time: string; allowed_programs: string; term_type: string; separate_by_gender: boolean }>
) {
...
export async function updateRentalSpace(
  accommodation_id: string,
  accommodationFields: Partial<{ name: string; location: string; manager_id: string }>,
  rentingFields: Partial<{ property_type: string; allow_shortterm_stay: boolean; allow_longterm_stay: boolean; minimum_stay_days: number; maximum_stay_days: number; security_deposit_required: boolean }>
) {
  if (Object.keys(accommodationFields).length > 0) {
    await supabaseAdmin
      .from('accommodation')
...
export async function updateUnit(unit_id: string, fields: Partial<{
  unit_number: string
  unit_type: string
  max_occupancy: number
  rental_fee: number
